...
def KL_divergence(p: DocLanguageModel, q: Dict[str, float])->float:
    ...
    p_values = np.array([p.get_term_prob(keys) for key in keys])
...
def main():
    ...
    for query in queries:
        ...
        results = []
        for doc_id in doc_ids_query:
            kl_div = KL_divergence(doc_lms[doc_id], relevance_model_probabilities)
            results.append((doc_id, 1-kl_div))
            
        results.sort(key=lambda x: x[1], reverse=True)
        for idx, result in enumerate(results):
            doc_id, score = result
            output_file.write(f"{query} Q0 {doc_id} {idx+1} {score:.4f} runid1\n")
        print(f"Processed Query: {query }\r")
